...
#überlassen. 
frauen.studiengang.bw <- ggplot(data=frauen,aes(x=major))
 
weight.bw <- frauen.studiengang.bw  + geom_boxplot(aes(x=major,y=height))
print(weight.bw)
...
frauen.studiengang.dichte <- (frauen.studiengang.bw + geom_density())
...
klinisch.sd <- sqrt(var(klinisch))
linkk.sd <- sqrt(var(linkk))
speech.sd <- sqrt(var(speech))
...
print( paste("Studiengang: Linguistik & Kommunikation","Mean:",mean(linkk$height),"SD:",linkk.sd) )
