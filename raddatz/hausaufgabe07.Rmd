...
Proportional mit jeweils einem größeren Prozentwert für die Durchfallenden als für die Ausgezeichneten; aber je nach SD mit unterschiedlichem Faktor; bei SD=5 sind die Noten <5 Punkte nicht einmal doppelt so viele wie die >12 Punkte, bei SD=3 sind die Durchfallenden mehr als 3 mal soviele wie die mit sehr guten Noten. 

Warum?

Die Verteilungskurven sind gegenüber dem Mittelwert leicht nach rechts verschoben und die ausgezeichneten Noten umfassen mit den Werten 13/14/15 einen Wert weniger als die mangelhaften 1/2/3/4. Bei der SD=5 ist die Kurtosis deutlich flacher als bei SD=3, so dass die Anzahl der immer noch exzellenten bzw. mangelhaften Benotungen zu den Rändern des Notenbereichs hin schneller abnimmt, insbesondere bei den einen kleineren (und extremeren) Bereich umfassenden ausgezeichneten Noten.
...
linksschief, da sich der Gipfel bei mehr guten Noten (bei einem System, das höhere Zahlen für bessere Noten verwendet) eher rechts befindet.
...
qplot(x=1:15,y=dsn(1:15, xi=c(12), omega=1, alpha=-3, log=FALSE),geom="line",xlab="Notenpunkte",ylab="Anzahl Studenten")
...
MitStudRel <- pnorm(9,mean=mu,sd=3) - pnorm(7,mean=mu,sd=3)
MitStudAbs <- MitStudRel*n
print( paste(MitStudAbs,"Studenten bekommen zwischen 7 und 9 Notenpunkten.") )
...
GutStudRel <- pnorm(10,mean=mu,sd=3,lower.tail=FALSE) 
GutStudAbs <- GutStudRel*n
print( paste(GutStudAbs,"Studenten bekommen zumindest 10 NP.") )
 
3. Wie viele Studenten bekommen weniger als 10 NP?

NGutStudRel <- pnorm(10,mean=mu,sd=3) 
NGutStudAbs <- NGutStudRel*n
print( paste(NGutStudAbs,"Studenten bekommen weniger als 10 Notenpunkte.") )    

4. Wie viele Studenten bekommen weniger als 8 NP?

NBefStudRel <- pnorm(8,mean=mu,sd=3) 
NBefStudAbs <- NBefStudRel*n
print( paste(NBefStudAbs,"Studenten bekommen weniger als 8 Notenpunkte.") )     
...
Der kritische Wert für einen einseitigen $z$-Test ist qnorm(0.94)=1.554774

Die kritischen Werte für einen zweiseitigen $z$-Test sind +/- qnorm(0.97)=1.880794 und -1.880794
...
Zweiseitiger Test, da die Richtung der allgemeiner Abweichung seiner Benotung nicht klar ist (?)
...
IdealDurch <- mean(1:15)
IdealSD <- 3

z7 <- sqrt(7)*((10-IdealDurch)/IdealSD)
z7

 
' Ergebnis 1.764 ist unterhalb von kritischem Wert für zweiseitigen Test, H0 kann bzw. muss nicht verworfen werden.
...
z50 <- sqrt(50)*((10-IdealDurch)/IdealSD)
z50

Ergebnis 4.714 ist weit über kritischem Wert für zweiseitigem (und einseitigem) Test.
...
Einseitiger Test, da dezidiert auf "zu streng" geprüft werden soll (anstelle von "zu streng oder zu mild")
...
z20 <- sqrt(20)*((7-IdealDurch)/IdealSD)
z20
...
z25 <- sqrt(25)*((7-IdealDurch)/IdealSD)
z25

Das ist ein signifikanter Unterschied, da der Betrag (1.667) des berechneten z-Werts über dem kritischen Wert für den einseitigen Test liegt. 
